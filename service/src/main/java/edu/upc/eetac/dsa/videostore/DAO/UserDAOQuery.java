...
public interface UserDAOQuery {
    ...
    public final static String ASSIGN_ROLE_REGISTERED = "insert into usuario_rol (usuarioid, role) values (UNHEX(?), 'registered')";
    public final static String GET_USER_BY_ID = "select hex(id) as id, loginid, email, fullname, saldo from usuario where id=unhex(?)";
    public final static String GET_USER_BY_USERNAME = "select hex(id) as id, loginid, email, fullname, saldo from usuario where loginid=?";
