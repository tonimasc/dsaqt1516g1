...
public interface AuthTokenDAOQuery {
    ...
    public final static String GET_USER_BY_TOKEN = "select hex(u.id) as id from usuario u, auth_tokens t where t.token=unhex(?) and u.id=t.usurioid";
    public final static String GET_ROLES_OF_USER = "select hex(usuarioid), role from user_roles where usuarioid=unhex(?)";
